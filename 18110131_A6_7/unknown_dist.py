#Q5 Random disturbunce with std=0.1
#SCARA 
#Imports
...
steps=200
Num=10
time=np.linspace(0,steps/Num,steps)
...
def dynamicSystem(T,y,D,C,Jm,B_eff,Q,Q_dot,trajectories,controller,Kp,Kd,disturbance):
    q1=y[0]
    q2=y[1]
    d3=y[2]
    q_dot1=y[3]
    ...
    Q_ddot=np.linalg.inv(Jm +D)@((U+disturbance[index]) - G - (C+B_eff)@Q_dot)
    q_ddot1=Q_ddot[0,0]
    q_ddot2=Q_ddot[1,0]
    d_ddot3=Q_ddot[2,0]
...
def plotSCARA(X,Xd,time,mainTitle,Q1,Q2,D3):
    ...
    anim.save('18110131_A5_SCARA_'+mainTitle+'.mp4')
...
if __name__=="__main__":
    ...
    sample = my_dist.rvs(size=steps+1)#Disturbance acts for 1s each sample
    ...
    while not(userInput == 5):
        ...
        if userInput==1:
            solution=scipy.integrate.solve_ivp(dynamicSystem ,[0, time[-1]],[Q_d0[0],Q_d0[1],Q_d0[2],0,0,0],t_eval=time, args=[D,C,Jm,B_eff,Q,Q_dot,trajectories, PD,Kp,Kd,sample])           
            maintitle="PD controller"
        elif userInput==2:
            solution=scipy.integrate.solve_ivp(dynamicSystem ,[0, time[-1]],[Q_d0[0],Q_d0[1],Q_d0[2],0,0,0],t_eval=time, args=[D,C,Jm,B_eff,Q,Q_dot,trajectories, PD_FF,Kp,Kd,sample])
            maintitle="PD controller + Feed Forward"
        elif userInput==3:
            solution=scipy.integrate.solve_ivp(dynamicSystem ,[0, time[-1]],[Q_d0[0],Q_d0[1],Q_d0[2],0,0,0],t_eval=time, args=[D,C,Jm,B_eff,Q,Q_dot,trajectories, PD_FF_disturbance,Kp,Kd,sample])
            maintitle="PD controller + Feed Forward + Disturbance Control"
        elif userInput==4:
            solution=scipy.integrate.solve_ivp(dynamicSystem ,[0, time[-1]],[Q_d0[0],Q_d0[1],Q_d0[2],0,0,0],t_eval=time, args=[D,C,Jm,B_eff,Q,Q_dot,trajectories, multiVariable,Kp,Kd,sample])
