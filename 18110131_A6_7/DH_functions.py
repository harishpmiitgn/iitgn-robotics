...
def Jacobian_EndPoint_DH(DH_param,P):
    ...
    J0_P=np.zeros([6,linkCount],dtype=float) #Jacobian matrix
...
def dynamics_equation_generator(DH_param,M,I,L):
    ...
    N=np.zeros([linkCount,linkCount],dtype=object)#The derivation of skew symmetric matrix for confirmation of dynamics
    #N= D_dot - 2C. Refer textbook for the formula for sumation with respect to i or j
    ...
    for k in range(linkCount):
        for i in range(linkCount):
            ...
            for j in range(linkCount):
                ...
                N[k,i]=N[k,i]+(sym.diff(D[i,j],Q[k])-sym.diff(D[k,j],Q[i]))*Q_dot[j]

    return[D,C,Q,Q_dot,N]

def manipulatorPos(DH_param):
    #Returns position of all coordinate axes origins 
    O=np.array([[0],[0],[0],[1]])
    pos=[]
    H_list=Transformation_matrices(DH_param)
    for H in H_list:
...
def endEffectorTrajectory(ini_pos,final_pos,ini_vel,final_vel,t_in,t_f):
    #Returns trajectory equations for x,yz in terms of time dependent cubic function using initial and final values
    #of positions and translation velocities.
    #ini_pos, etc. are list of 3 elements with 1st element referring to x axis and so on
    #P= a0 +a1t +a2t^2 + a3t^3
    A_inv=np.linalg.inv(np.array([[1, t_in,t_in**2,t_in**3],[0,1,2*t_in,3*t_in**2],[1, t_f,t_f**2,t_f**3],[0,1,2*t_f,3*t_f**2]]))
    trajectories=[]
    t=sym.Symbol('t')

    for i in range(3):
        #0->x 1->y 2->z
        b= np.array([[ini_pos[i]],[ini_vel[i]],[final_pos[i]],[final_vel[i]]])
        coeff= A_inv@b
        trajectory=coeff[0,0]+coeff[1,0]*t+coeff[2,0]*(t**2)+coeff[3,0]*(t**3)
...
if __name__=="__main__":
    ...
    [D,C,Q,Q_dot,N]=dynamics_equation_generator(DH_param,M,I,L)
    print(sym.simplify(D))
    print(sym.simplify(C))
    N_arr=np.array(sym.simplify(N))
    
    print(N_arr) # this shows that N matrix is skew symetric hence the code is working perfectly and equations are correct.
