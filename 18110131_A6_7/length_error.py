...
L_actual=[0.3,0.2,0.5]#3rd one is to set limit on Prismatic length
L_error=[0.24, 0.2, 0.5] #Error in the first length 20% reduced taken in calculation
# I=[(1/12)*M[0]*L[0]**2 ,(1/12)*M[1]*L[1]**2 ,0.5*M[2]*r**2]
def make_Inertia(L):
    return [np.array([[0.5*M[0]*r**2,0,0],[0,(1/12)*M[0]*L[0]**2,0],[0,0,(1/12)*M[0]*L[0]**2]]),
...
def SCARA_Jacobian_dot(Q,Q_dot,L):
    q1=Q[0]
    q2=Q[1]
    d3=Q[2]
    q1_dot=Q_dot[0]
...
def jointCalculations(trajectories,T,t,L):
    x=[]
    ...
    q=inverseKinematics(x,L)
    DH_param=SCARA_DH_gen(q,L)
    P=np.array([[0],[0],[0],[1]])
    [J,P0]=dh.Jacobian_EndPoint_DH(DH_param,P)
    J_inv= np.linalg.pinv(J)
    q_dot=J_inv@x_dot

    J_dot=SCARA_Jacobian_dot(q,q_dot,L)
...
def dynamicSystem(T,y,D,C,Dd,Cd,Jm,B_eff,Q,Q_dot,trajectories,controller,Kp,Kd):
    q1=y[0]
    q2=y[1]
    d3=y[2]
    q_dot1=y[3]
    ...
    [Q_d, Q_d_dot, Q_d_ddot]= jointCalculations(trajectories,T,t,L_error)
    ...
    Q_ddot=np.linalg.inv(Jm +D)@(U - G - (C+B_eff)@Q_dot)
    q_ddot1=Q_ddot[0,0]
    q_ddot2=Q_ddot[1,0]
    d_ddot3=Q_ddot[2,0]
...
def updatePlot(i,Q1,Q2,D3,start,end,ax,ax2):
    ...
    q2=Q2[i]
    d3=D3[i]
    DH_param=SCARA_DH_gen([q1,q2,d3],L_actual)
...
def plotSCARA(X,Xd,time,mainTitle,Q1,Q2,D3):
    ...
    anim.save('18110131_A4_SCARA_'+mainTitle+'.mp4')
    

def forward_kinematics(q1,L):
    DH_param=SCARA_DH_gen(q1,L)
    P=np.array([[0],[0],[0],[1]])
...
if __name__=="__main__":
    ...
    DH_param=SCARA_DH_gen(Q,L_actual)
    [D,C,Q,Q_dot,N]=dh.dynamics_equation_generator(DH_param,M,make_Inertia(L_actual),L_actual)
    ...
    DH_param=SCARA_DH_gen(Q,L_error)
    [De,Ce,Qe,Q_dote,Ne]=dh.dynamics_equation_generator(DH_param,M,make_Inertia(L_error),L_error)
    [trajectories,t]=dh.endEffectorTrajectory(start,end,start_vel,end_vel,time[0],time[-1])
    
    Q_d0= inverseKinematics(start,L_error) 
    size=len(time)
    coord=[]
    for i in range(size):
        x=[]
    ...
    while not(userInput == 5):
        ...
        if userInput==1:
            solution=scipy.integrate.solve_ivp(dynamicSystem ,[0, time[-1]],[Q_d0[0],Q_d0[1],Q_d0[2],0,0,0],t_eval=time, args=[D,C,De,Ce,Jm,B_eff,Q,Q_dot,trajectories, PD,Kp,Kd])           
            maintitle="PD controller"
        elif userInput==2:
            solution=scipy.integrate.solve_ivp(dynamicSystem ,[0, time[-1]],[Q_d0[0],Q_d0[1],Q_d0[2],0,0,0],t_eval=time, args=[D,C,De,Ce,Jm,B_eff,Q,Q_dot,trajectories, PD_FF,Kp,Kd])
            maintitle="PD controller + Feed Forward"
        elif userInput==3:
            solution=scipy.integrate.solve_ivp(dynamicSystem ,[0, time[-1]],[Q_d0[0],Q_d0[1],Q_d0[2],0,0,0],t_eval=time, args=[D,C,De,Ce,Jm,B_eff,Q,Q_dot,trajectories, PD_FF_disturbance,Kp,Kd])
            maintitle="PD controller + Feed Forward + Disturbance Control"
        elif userInput==4:
            solution=scipy.integrate.solve_ivp(dynamicSystem ,[0, time[-1]],[Q_d0[0],Q_d0[1],Q_d0[2],0,0,0],t_eval=time, args=[D,C,De,Ce,Jm,B_eff,Q,Q_dot,trajectories, multiVariable,Kp,Kd])
        ...
        for i in range(len(Q1)):
            P=forward_kinematics([Q1[i],Q2[i],D3[i]],L_actual)
